...
[図表挿入予定: データ収集システムの全体アーキテクチャ図 - 収集スケジューラからデータ保存システムまでの情報の流れを示す図]

## n8nによる収集スケジューラの実装

n8nを活用して、情報源ごとに最適なスケジュールでデータを収集するシステムを実装します。

[図表挿入予定: スケジューラとディスパッチャーの関係図 - マスタースケジューラがジョブを生成し、ディスパッチャーが各コネクタに振り分ける流れを示す図]
...
[図表挿入予定: 各コネクタの処理フロー図 - Web、RSS、APIの各コネクタがデータを収集・処理・保存するまでの流れを示す図]
...
トリプルパースペクティブ型戦略AIレーダーのデータ収集システムを構築するための具体的なステップを以下に示します。これらのステップに従うことで、基本的なデータ収集システムを段階的に実装することができます。
...
1. **n8nの設定**
   - 前のセクションで設定したn8n環境を使用
   - 必要なノードの確認（HTTP Request、Function、PostgreSQL、Split In Batches など）
...
1. **マスタースケジューラワークフロー**
   - n8nのWebインターフェース（http://localhost:5678）にアクセス
   - 新規ワークフローを作成し「Master Scheduler」と名付ける
   - 本セクションで紹介したワークフローのノードを順に追加
   - スケジュールトリガーを設定（5分ごとなど）
   - 各ノードのパラメータを設定（データベース接続情報など）
   - ワークフローを保存し、アクティブ化

2. **クロールディスパッチャーワークフロー**
   - 同様に「Crawl Dispatcher」ワークフローを作成
   - Webhookトリガーを設定（/crawl-dispatcher パス）
   - 本セクションで紹介したノード構成を実装
   - データベース接続を設定し、テスト実行

### 3. コネクタの実装

各情報源タイプに対応したコネクタを実装します：

1. **Webクローラー**
   - 「Web Crawler」ワークフローを作成
   - Webhookトリガーを設定（/web-crawler パス）
   - HTTP Requestノードでウェブページを取得
   - Functionノードでコンテンツ抽出ロジックを実装
   - 保存ワークフローへの連携を設定

2. **RSSフィードコネクタ**
   - 「RSS Crawler」ワークフローを作成
   - RSS Feed Readノードを使用してフィード取得
   - 各アイテムの処理と保存を実装

3. **APIコネクタ**
   - 「API Crawler」ワークフローを作成
   - 対象APIに合わせたパラメータ設定
   - レスポンス処理と保存を実装

### 4. 前処理パイプラインの実装

収集したデータの前処理を行うワークフローを実装します：

1. **コンテンツ保存ワークフロー**
   - 「Save Content」ワークフローを作成
   - 重複チェックと保存ロジックを実装
   - 画像の抽出と保存を設定

2. **テキスト正規化ワークフロー**
   - 「Process Content」ワークフローを作成
   - テキスト正規化、言語検出、エンティティ抽出を実装
   - 処理結果をデータベースに保存

3. **トピック関連付けワークフロー**
   - 「Link Content to Topics」ワークフローを作成
   - キーワードとエンティティに基づく関連性計算を実装
   - 関連トピックの保存を設定

### 5. テストと最適化

実装したシステムのテストと最適化を行います：

1. **テスト実行**
   - 各ワークフローを個別にテスト実行
   - マスタースケジューラを手動トリガーして全体フローをテスト
   - 収集結果をデータベースで確認
...
データ収集システム構築時によく発生する問題と解決策を紹介します。

### 1. クローリング関連の問題

1. **接続エラーとタイムアウト**
   - 問題: ウェブサイトへのアクセスがタイムアウトする
   - 解決策: HTTP Requestノードのタイムアウト設定を増加（10000ms→30000ms）
   - 解決策: User-Agentヘッダーを追加して正規のブラウザとして認識されるようにする
   ```javascript
   {
     "headers": {
...
2. **アクセス制限への対応**
   - 問題: IPブロックやCAPTCHAによるアクセス制限
   - 解決策: クロール間隔を長く設定（Wait ノードを使用）
   - 解決策: プロキシローテーションの導入
   ```javascript
   // プロキシローテーションの例
   const proxies = [
   ...
   const proxyIndex = Date.now() % proxies.length;
   const proxy = proxies[proxyIndex];
   ```

3. **コンテンツ抽出の失敗**
   - 問題: ウェブサイトの構造変更によりコンテンツ抽出が失敗
   - 解決策: より堅牢なセレクタを使用（複数のセレクタを試行）
   ```javascript
   // 複数セレクタを試行する例
   const selectors = ['article', '.content', '.post', '#main-content'];
   let content = '';
...
2. **メモリ使用量の最適化**
   - 問題: 大きなデータセットでメモリ不足エラーが発生
   - 解決策: ストリーミング処理の導入
   - 解決策: 不要なデータの早期フィルタリング
   ```javascript
   // 早期フィルタリングの例
   const filteredData = data.filter(item => {
     // 必要な条件を満たすアイテムのみ保持
     return item.relevance > 0.5 && item.date > lastProcessedDate;
   });
   ```
...
1. **水平スケーリング**
   - 複数のn8nインスタンスでワークロードを分散
   - 情報源をグループ化し、各インスタンスに割り当て
   ```javascript
   // 情報源のグループ化例
   const sourceGroups = {
     'instance1': sources.filter(s => s.id % 3 === 0),
     'instance2': sources.filter(s => s.id % 3 === 1),
     'instance3': sources.filter(s => s.id % 3 === 2)
   };
   ```

2. **キューイングシステムの導入**
   - RabbitMQやRedisを使用したジョブキューの実装
   - クロールジョブの優先順位付けと制御
   ```javascript
   // キューイングシステム連携例
   const queueJob = {
   ...
   // RabbitMQへの送信（n8nのRabbitMQノードを使用）
   ```

3. **データベースの最適化**
   - 定期的なメンテナンス（VACUUM、ANALYZE）の実施
   - パーティショニングの導入（時間ベース）
   ```sql
   -- テーブルパーティショニングの例
   ...
   CREATE TABLE content_y2023m01 PARTITION OF content_partitioned
     FOR VALUES FROM ('2023-01-01') TO ('2023-02-01');
   ```

これらのトラブルシューティングと最適化のヒントを活用することで、より堅牢で効率的なデータ収集システムを構築することができます。

## まとめ

このセクションでは、トリプルパースペクティブ型戦略AIレーダーのデータ収集システムの実装方法について解説しました。n8nを活用することで、多様な情報源からデータを効率的に収集し、前処理を行うシステムを構築することができます。

収集スケジューラ、各種コネクタ、コンテンツ保存と前処理パイプラインを組み合わせることで、高品質なデータセットを自動的に構築し、分析に活用することができます。さらに、実装ステップのガイドとトラブルシューティングのヒントにより、実際のシステム構築をスムーズに進めることができます。
