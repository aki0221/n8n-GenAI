...
*エラーケース別の対応策表*: (ここに主要なエラーケースとその対応策をまとめた表を記述。例：| エラーケース | 原因 | 対応策 | n8nノード例 | |---|---|---|---| | データ欠損 | 入力データ不備 | デフォルト値補完 or エラー通知 | Function, If, Email | | 計算エラー | ゼロ除算、型不一致 | 入力値チェック、try-catch構文 | Function | | API接続エラー | 外部API障害 | リトライ処理、エラー通知 | HTTP Request, Wait, Slack |)
